...
This satisfies the core requirement of Task 1: two-way retrieval with the mandated SaProt protein tower and a literature-motivated molecular tower, runnable in a Colab environment.

---

## 8. Optional Improvements (If Required for “ColabService” Submission)
If the grading rubric emphasizes an “online service” interface, the next minimal steps are:

- Add a Gradio UI in Colab with two tabs (P→M and M→P).
- Save and load the learned projection weights (`state_dict`) to support “load weights → retrieve” workflow.
- Report M→P test metrics (Recall@K / MRR@K) symmetrically.
